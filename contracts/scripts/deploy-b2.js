...
const { getBalanceAsNumber, deployContractsB2 } = require("./utils");
...
async function main() {
    ...
    const layer1 = hre.network.config.layer1;
    const aTokenAddress = addressesData['aToken'][layer1];
    console.log('layer1', layer1, 'aTokenAddress', aTokenAddress);
    ...
    const { tellor, subscriptionChecker, gameMaster } = await deployContractsB2({ subscriptionChecker: [aTokenAddress, requestId, threshold, tipIncrement] });
